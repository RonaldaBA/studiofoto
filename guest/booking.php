...
if(isset($_SESSION['guest_customer'])){
    $id_customer = $_SESSION['guest_customer'];
}
...
if (isset($_POST['simpan'])) {
    ...
    if(mysqli_num_rows($cek) > 0){
        ...
        $cust = mysqli_fetch_assoc($cek);

        $id_customer = $cust['id_customer'];
        $qGuest = mysqli_query(
            $connection,
                "SELECT * FROM guest_account
                WHERE id_customer='$id_customer'
                AND status='active'
                LIMIT 1"
            );

        if(mysqli_num_rows($qGuest) > 0){
                $guest = mysqli_fetch_assoc($qGuest);

                $_SESSION['guest_token'] = $guest['guest_token'];
                $_SESSION['guest_customer'] = $id_customer;
            }
...
function loadJam() {
    ...
    fetch(`booking.php?ajax=cek_jam&tanggal=${tanggal}&paket=${paket.id.replace('qty_','')}`)
        .then(res => res.json())
        .then(data => {
...
function loadBookedDates() {
  fetch(`booking.php?ajax=cek_tanggal&bulan=${currentMonth+1}&tahun=${currentYear}`)
    .then(res => res.json())
    .then(data => {
      bookedDates = data;
      renderCalendar();
